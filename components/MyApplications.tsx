    ...
    <div className="flex-1 overflow-y-auto bg-slate-950 p-6 pb-24 no-scrollbar">
      ...
      <div className="flex bg-slate-900 rounded-2xl p-1 mb-6 border border-slate-800">
        <button 
          ...
          className={`flex-1 py-3 text-sm font-bold rounded-xl transition-all ${activeTab === 'applied' ? 'bg-blue-600 text-white' : 'text-slate-500'}`}
        >
          Inscritas ({appliedJobs.length})
        </button>
        <button 
          onClick={() => setActiveTab('saved')}
          className={`flex-1 py-3 text-sm font-bold rounded-xl transition-all ${activeTab === 'saved' ? 'bg-blue-600 text-white' : 'text-slate-500'}`}
        >
